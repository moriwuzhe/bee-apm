...
import { Plus, Search, X, Download, RefreshCw, ExternalLink, Settings } from "lucide-react";
      ...
      try {
        ...
        if (response.data && response.data.length > 0) {
          setApps(response.data.map((a: any) => ({
            ...
            jvm: a.javaVersion || "JDK8", heap: Math.min(Math.round((a.heapUsage || 0) * 100), 100),
    ...
    <MainLayout title="应用管理">
      <div data-cmp="Applications" className="space-y-4">
              ...
              <tbody>
                {paginatedData.map((app) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-xs text-muted-foreground">{app.runtime}</td>
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2">
                        <div className="w-20 h-1.5 rounded-full overflow-hidden bg-gray-500/20">
                          <div className="h-full rounded-full transition-all" style={{ width: `${app.heap}%`, background: app.heap > 85 ? "#FF4D4F" : app.heap > 70 ? "#FFAA00" : "#00D68F" }} />
                        </div>
                        <span className="text-xs" style={{ color: app.heap > 85 ? "#FF4D4F" : app.heap > 70 ? "#FFAA00" : "#00D68F" }}>{app.heap}%</span>
                      </div>
                    </td>
                    <td className="px-3 py-2.5 text-xs text-white">{app.inst}</td>
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-1">
                        <TechButton variant="ghost" size="xs" icon={<ExternalLink size={11} />} onClick={() => handleView(app)}>查看</TechButton>
        ...
        {showDetailModal && viewingApp && (
          <div className="fixed inset-0 flex items-center justify-center bg-black/70 z-[200]" onClick={() => setShowDetailModal(false)}>
            <div className="w-[400px] rounded-xl p-6 bg-card border border-border" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
                <div className="text-lg font-semibold text-white">应用详情</div>
                <button onClick={() => setShowDetailModal(false)} className="text-gray-400 hover:text-white"><X size={16} /></button>
              </div>
              <div className="space-y-4">
                <div className="p-3 rounded-md bg-muted">
                  <div className="text-xs font-medium mb-2 text-muted-foreground">基础信息</div>
                  <div className="space-y-2">
                    {([["应用名称", viewingApp.name], ["所属项目", viewingApp.project], ["状态", statusLabels[viewingApp.status]?.label], ["实例IP", viewingApp.ip], ["Agent版本", viewingApp.agent], ["JDK版本", viewingApp.jvm], ["运行时长", viewingApp.runtime], ["实例数", viewingApp.inst]] as [string, any][]).map(([k, v]) => (
                ...
                <div className="p-3 rounded-md bg-muted">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-2 rounded-full overflow-hidden bg-gray-500/20">
                      <div className="h-full rounded-full transition-all" style={{ width: `${viewingApp.heap}%`, background: viewingApp.heap > 85 ? "#FF4D4F" : viewingApp.heap > 70 ? "#FFAA00" : "#00D68F" }} />
                    </div>
                    <span className="text-xs" style={{ color: viewingApp.heap > 85 ? "#FF4D4F" : viewingApp.heap > 70 ? "#FFAA00" : "#00D68F" }}>堆内存 {viewingApp.heap}%</span>
                  </div>
                </div>
                <div className="flex gap-2">
                  <TechButton variant="secondary" size="sm" onClick={() => setShowDetailModal(false)}>关闭</TechButton>
                  <TechButton variant="primary" size="sm" icon={<Settings size={12} />} onClick={() => { setShowDetailModal(false); handleEdit(viewingApp); }}>编辑应用</TechButton>
                </div>
              </div>
            </div>
          </div>
