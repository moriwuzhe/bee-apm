...
import { Plus, Search, Monitor, Clock, ExternalLink, Settings, X } from "lucide-react";
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full h-9 px-3 rounded-md text-sm bg-input border border-border text-white outline-none" placeholder="例如: order-service" />
                  </div>
                  <div>
                    <label className="text-xs font-medium block mb-1.5 text-muted-foreground">状态</label>
                    <select value={formData.status} onChange={(e) => setFormData({ ...formData, status: e.target.value })} className="w-full h-9 px-3 rounded-md text-sm text-white outline-none appearance-none cursor-pointer" style={{ background: "#1E293B", border: "1px solid rgba(148, 163, 184, 0.12)" }}>
                      <option value="online" style={{ color: "#fff", background: "#1E293B" }}>在线</option><option value="warning" style={{ color: "#fff", background: "#1E293B" }}>警告</option><option value="error" style={{ color: "#fff", background: "#1E293B" }}>异常</option><option value="offline" style={{ color: "#fff", background: "#1E293B" }}>离线</option>
                    </select>
                  </div>
                  <div><label className="text-xs font-medium block mb-1.5 text-muted-foreground">实例IP</label><input type="text" value={formData.ip} onChange={(e) => setFormData({ ...formData, ip: e.target.value })} className="w-full h-9 px-3 rounded-md text-sm bg-input border border-border text-white outline-none" placeholder="例如: 192.168.1.10" /></div>
                  <div><label className="text-xs font-medium block mb-1.5 text-muted-foreground">Agent版本</label><input type="text" value={formData.agentVersion} onChange={(e) => setFormData({ ...formData, agentVersion: e.target.value })} className="w-full h-9 px-3 rounded-md text-sm bg-input border border-border text-white outline-none" placeholder="例如: v2.4.1" /></div>
                  <div><label className="text-xs font-medium block mb-1.5 text-muted-foreground">JDK版本</label><input type="text" value={formData.jvmVersion} onChange={(e) => setFormData({ ...formData, jvmVersion: e.target.value })} className="w-full h-9 px-3 rounded-md text-sm bg-input border border-border text-white outline-none" placeholder="例如: JDK17" /></div>
                  <div><label className="text-xs font-medium block mb-1.5 text-muted-foreground">堆内存使用(%)</label><input type="number" min="0" max="100" value={formData.heapUsage} onChange={(e) => setFormData({ ...formData, heapUsage: parseInt(e.target.value) || 0 })} className="w-full h-9 px-3 rounded-md text-sm bg-input border border-border text-white outline-none" /></div>
                  <div><label className="text-xs font-medium block mb-1.5 text-muted-foreground">运行时长</label><input type="text" value={formData.uptime} onChange={(e) => setFormData({ ...formData, uptime: e.target.value })} className="w-full h-9 px-3 rounded-md text-sm bg-input border border-border text-white outline-none" placeholder="例如: 12d 4h" /></div>
                  <div><label className="text-xs font-medium block mb-1.5 text-muted-foreground">实例数</label><input type="number" min="1" value={formData.instanceCount} onChange={(e) => setFormData({ ...formData, instanceCount: parseInt(e.target.value) || 1 })} className="w-full h-9 px-3 rounded-md text-sm bg-input border border-border text-white outline-none" /></div>
                </div>
