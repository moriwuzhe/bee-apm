...
* **Key Changes:**
  * **品牌与模块重命名**：将项目从 `BeeAPM` 全面重命名为 **LT监控系统 (LT Monitor)**，重构了所有核心模块（如 `bee-apm-agent` 变为 `lt-agent`，`bee-apm-server` 变为 `lt-server-apm`）。
...
| --- | --- | --- |
| `bee-apm-agent` | `lt-agent` | 匹配 LT Monitor 品牌 |
| `bee-apm-server` | `lt-server-apm` | 服务端模块重命名 |
| `bee-apm-ui` | `lt-ui-old` / `lt-monitor-ui-server` | UI 模块重命名及归档 |
...
* ⚠️ **Breaking Changes (破坏性变更):** 
  * 所有的启动脚本、打包产物名称、以及旧版的 `bee-apm` 配置项均已失效。如果现有环境依赖旧的 ZK 节点或旧包名进行自定义插件开发，将发生编译或运行时错误。
