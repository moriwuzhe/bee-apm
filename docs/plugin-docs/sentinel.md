...
| 参数名称 | 默认值 | 说明 |
|---------|--------|------|
| bee.plugin.sentinel.enable | true | 是否开启Sentinel插件 |
| bee.plugin.sentinel.collect.blocked.only | false | 是否只采集被拒绝的请求（默认采集所有请求） |
| bee.plugin.sentinel.collect.metrics | true | 是否采集Sentinel metrics指标 |
| bee.plugin.sentinel.metrics.interval | 60000 | 指标采集间隔（毫秒，默认1分钟） |
...
```properties
bee.plugin.sentinel.collect.blocked.only=true
```
...
```properties
# 调整指标采集间隔为5分钟
bee.plugin.sentinel.metrics.interval=300000
```
不需要指标采集可以关闭：
```properties
bee.plugin.sentinel.collect.metrics=false
```
...
## 常见问题
### Q: 和Sentinel自带的监控冲突吗？
A: 不冲突，Bee-APM的Sentinel插件是基于字节码拦截，不会影响Sentinel本身的功能和自带监控。
...
A: 可以通过配置资源名黑名单：
```properties
bee.plugin.sentinel.exclude.resources=web_ignore,test_*,/health
```
支持通配符 `*` 匹配多个资源。
