...
$DemoDir = Join-Path $BaseDir "lt-agent-test\bee-apm-sb-demo"
...
$App = $env:BEE_APP
if ([string]::IsNullOrWhiteSpace($App)) { $App = "bee-preprod-demo" }
$EnvName = $env:BEE_ENV
...
$TmpCfg = Join-Path $TargetDir "bee-agent-preprod.yml"
...
java -Xms256m -Xmx256m `
  ...
  "-Dremote.ports=$Port" `
  "-Dmax.counter=0" `
  "-Dbee.demo.load.enable=$LoadEnable" `
  "-Dbee.demo.load.qps=$LoadQps" `
  "-Dbee.demo.load.threads=$LoadThreads" `
  "-Dbee.demo.load.durationSeconds=$LoadDurationSeconds" `
