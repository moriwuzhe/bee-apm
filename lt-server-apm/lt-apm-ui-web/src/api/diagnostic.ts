...
export async function fetchAgentConnections(): Promise<AgentConnection[]> {
  const res = await diagHttp.get<ApiResult<AgentConnection[]>>('/diag/agent/version/detail')
  return unwrap(res.data) || []
}

export async function searchAgentConnections(agentId: string): Promise<AgentConnection[]> {
  const res = await diagHttp.get<ApiResult<AgentConnection[]>>('/diag/agent/version/search', { params: { agentId } })
  return unwrap(res.data) || []
}
