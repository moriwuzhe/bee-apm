...
async function openConfig(row: any) {
  ...
  try {
    ...
    const cfg = (res.data as any)?.result?.config ?? (res.data as any)?.config
    configContent.value = cfg || '# 暂无配置'
  } catch (e: any) {
  ...
  <el-dialog v-model="installDialogVisible" title="Agent 下载与安装指引" width="650px" append-to-body>
    <div style="line-height: 1.6;">
      ...
      <div style="background:#282c34;color:#abb2bf;padding:12px;border-radius:4px;font-family:monospace;margin:8px 0;word-break:break-all;">
        curl -sSL http://&lt;your-server-ip&gt;:8081/api/agent/install.sh | bash -s -- &lt;您的应用名&gt; http://&lt;your-server-ip&gt;:8081
      </div>
      ...
      <div style="background:#282c34;color:#abb2bf;padding:12px;border-radius:4px;font-family:monospace;margin:8px 0;word-break:break-all;">
        -javaagent:/opt/lt-monitor/agent/lt-agent.jar -Dlt.config=/opt/lt-monitor/agent/config.yml -Dlt.app=&lt;您的应用名&gt; -Dlt.env=prod -Dlt.inst=&lt;实例名&gt;
      </div>
    </div>
    <template #footer>
