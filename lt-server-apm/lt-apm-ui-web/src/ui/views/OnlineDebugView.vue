                ...
                <el-form-item label="目标节点" required>
                  <el-select v-model="debugForm.targetAgentId" placeholder="选择回放的隔离节点 (避免选择生产核心节点)" style="width: 100%">
                    <el-option
                      v-for="a in agents"
                      :key="a.agentId"
                      :label="a.agentId + (a.active ? ' (在线)' : ' (离线)')"
                      :value="a.agentId"
                      :disabled="!a.active || !a.writable"
                    />
                  </el-select>
                </el-form-item>
              </el-col>
        ...
        <el-card shadow="never" class="result-card" v-if="debugResult || debugRunning">
          <template #header>
            <div class="card-header">
              ...
              <el-button v-if="debugResult" size="small" type="primary" link @click="$router.push({ name: 'logger', query: { gid: selectedRequest?.gid } })"
                >查看关联日志</el-button
              >
            </div>
          </template>
