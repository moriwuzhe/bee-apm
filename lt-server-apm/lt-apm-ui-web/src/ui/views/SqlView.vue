...
function tableRowClassName({ row }: { row: SqlRow }) {
  if (row.error) {
    return 'error-row'
  }
...
function exportToExcel() {
  ...
  XLSX.writeFile(wb, `SQL数据_${new Date().toISOString().slice(0,10)}.xlsx`)
  ElMessage.success('导出成功')
}
</script>

<template>
  <PageShell title="SQL查询" :subtitle="subtitle || undefined">
    <template #actions>
      <div class="actions">
