...
public class EsDataAccessor {
    ...
    private static final Logger logger = LoggerFactory.getLogger(EsDataAccessor.class);
    ...
    public List<AlertRow> searchAlerts(String app, int limit) throws Exception {
        if (restHighLevelClient == null) {
            logger.debug("Elasticsearch not enabled, returning empty alert list");
            return new ArrayList<>();
        }
        
        try {
            SearchRequest searchRequest = new SearchRequest(INDEX_PATTERN_ALERT);
            SearchSourceBuilder sourceBuilder = new SearchSourceBuilder();
            ...
            for (SearchHit hit : response.getHits().getHits()) {
                ...
                row.setId(getStringValue(sourceMap, "id"));
                row.setTime(getLongValue(sourceMap, "time"));
                row.setApp(getStringValue(sourceMap, "app"));
                row.setUrl(getStringValue(sourceMap, "url"));
                row.setGid(getStringValue(sourceMap, "gid"));
                row.setAlertType(getStringValue(sourceMap, "alertType"));
                row.setMessage(getStringValue(sourceMap, "message"));
                row.setStatus(getStringValue(sourceMap, "status"));
                result.add(row);
            }
            
            return result;
        } catch (Exception e) {
            logger.error("Failed to search alerts", e);
            throw e;
        }
    }
