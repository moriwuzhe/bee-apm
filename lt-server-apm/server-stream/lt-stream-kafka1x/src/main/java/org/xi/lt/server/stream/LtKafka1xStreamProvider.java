...
public class LtKafka1xStreamProvider extends AbstractStreamProvider {
    private final String DEF_DESERIALIZER = "org.apache.kafka.common.serialization.StringDeserializer";
    private static final Logger logger = LoggerFactory.getLogger(LtKafka1xStreamProvider.class);
     ...
     * 拉取超时时间，默认500ms
     */
    private int POLL_TIMEOUT = 1000;

    @Override
    public void start() {
        IDLE_SLEEP = ConfigHolder.getPropInt("lt.provider.kafka1x.idleSleep", 500);
        //拉取超时时间，默认1000ms
        POLL_TIMEOUT = ConfigHolder.getPropInt("lt.provider.kafka1x.pollTimeout", 1000);
        logger.info("LtKafka1xStreamProvider start ............................................");
        initKafkaConfig();
        String[] topics = ConfigHolder.getProperty("lt.provider.kafka1x.topics").split(",");
        int idx = 0;
        for (String topic : topics) {
            ...
            t.setName("lt-kafka-" + threadNumber.getAndIncrement());
            t.start();
        }
    }
    ...
    private void initKafkaConfig() {
        ...
        Properties props = ConfigHolder.getProperties("lt.provider.kafka1x.kafkaConfig", true);
        ...
        if (!props.containsKey(ConsumerConfig.GROUP_ID_CONFIG)) {
            props.put(ConsumerConfig.GROUP_ID_CONFIG, "lt-consumer-group");
        }
        ...
        while (true) {
            try {
                ...
                for (int i = 0; i < count; i++) {
                    //QueueStreamHandler里的队列如果满了，会阻塞住
                    write(new Stream(cacheMsgList.get(i)));
                }
            } catch (Throwable e) {
                logger.error("", e);
