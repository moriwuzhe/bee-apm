package org.xi.lt.server.web.infrastructure.diag.replay;
...
public class RequestReplayService {
    ...
    public RequestSnapshot load(String requestId) {
        Object req = searchById(reqIndex, requestId);
        if (req == null) return null;
        Object reqb = searchById(reqbIndex, requestId);
        Object reqh = searchById(reqhIndex, requestId);
        Object rp = searchById(rpIndex, requestId);
    ...
    public ReplayResult replay(RequestSnapshot snapshot, String targetBaseUrl, boolean includeAuthHeaders, boolean includeCookieHeaders, int responseMaxChars) throws IOException {
        ...
        Object headersObj = sanitizeHeaders(snapshot.getHeaders(), includeAuthHeaders, includeCookieHeaders);
        Map headers = headersObj instanceof Map ? (Map) headersObj : Collections.emptyMap();
        ...
        for (Object o : headers.entrySet()) {
            ...
            rb.addHeader(k, String.valueOf(e.getValue()));
        }
    ...
    private Object searchById(String indexPattern, String requestId) {
        try {
    ...
    private static Object sanitizeHeaders(Object headersObj, boolean includeAuthHeaders, boolean includeCookieHeaders) {
        Map headers = headersObj instanceof Map ? (Map) headersObj : null;
    ...
    private static String findHeader(Object headersObj, String keyLower) {
        Map headers = headersObj instanceof Map ? (Map) headersObj : null;
        if (headers == null || headers.isEmpty()) return null;
        for (Object o : headers.entrySet()) {
            Map.Entry e = (Map.Entry) o;
            if (e.getKey() == null) continue;
            if (String.valueOf(e.getKey()).trim().toLowerCase().equals(keyLower)) return e.getValue() == null ? null : String.valueOf(e.getValue());
        }
        return null;
    }
    ...
    public static final class RequestSnapshot {
        ...
        static RequestSnapshot from(Object reqRaw, Object reqbRaw, Object reqhRaw, Object rpRaw) {
            Map req = (Map) asMap(map(reqRaw));
            Map reqb = (Map) asMap(map(reqbRaw));
            Map reqh = (Map) asMap(map(reqhRaw));
            Map rp = (Map) asMap(map(rpRaw));
            RequestSnapshot s = new RequestSnapshot();
            s.id = str(req.get("id"));
            s.ip = str(req.get("ip"));
            s.port = str(req.get("port"));
            Map tags = (Map) asMap(map(req.get("tags")));
            s.url = str(tags.get("url"));
            s.method = str(tags.get("method"));
            if (reqb != null && !reqb.isEmpty()) {
                Map t = (Map) asMap(map(reqb.get("tags")));
                s.body = str(t.get("body"));
            }
            if (reqh != null && !reqh.isEmpty()) {
                Map t = (Map) asMap(map(reqh.get("tags")));
                String hs = str(t.get("headers"));
                s.headers = parseStringMap(hs);
            }
            if (rp != null && !rp.isEmpty()) {
                Map t = (Map) asMap(map(rp.get("tags")));
        ...
        private static Object parseStringMap(String json) {
        ...
        private static Object asMap(Object o) {
            if (o instanceof Map) return o;
            return new LinkedHashMap();
        }
