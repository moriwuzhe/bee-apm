...
import org.xi.lt.ui.common.LtConst;
import org.xi.lt.ui.common.LtUtils;
...
public class EsJestClient {
    ...
    private static HttpClientConfig.Builder buildConfig(HttpClientConfig.Builder builder){
        Properties properties = ConfigHolder.getProperties(LtConst.KEY_ELASTICSEARCH,true);
        properties.remove(LtConst.KEY_ES_HTTP_URL);
        properties.remove(LtConst.KEY_ES_PASSWORD);
        properties.remove(LtConst.KEY_ES_USERNAME);
    ...
    private static void initJestClient(){
        String[] urls = ConfigHolder.getProperty(LtConst.KEY_ELASTICSEARCH + "." + LtConst.KEY_ES_HTTP_URL).split(",");
        HttpClientConfig.Builder builder = new  HttpClientConfig.Builder(Arrays.asList(urls));
        String userName = ConfigHolder.getProperty(LtConst.KEY_ELASTICSEARCH + "." + LtConst.KEY_ES_USERNAME);
        String password = ConfigHolder.getProperty(LtConst.KEY_ELASTICSEARCH + "." + LtConst.KEY_ES_PASSWORD);
    ...
    public SearchResult search(Map<String,Object> params,String mapId,String indexPrefix) throws IOException{
        String[] indices = LtUtils.getIndices(indexPrefix, params);
        Map<String, String> args = new HashMap<>();
        args.put("beginTime", LtUtils.getBeginTime(params).toString());
        args.put("endTime", LtUtils.getEndTime(params).toString());
