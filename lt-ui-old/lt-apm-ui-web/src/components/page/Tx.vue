                    ...
                    <el-col :span="7">
                        <el-form-item label="排序">
                            <el-select v-model="form.sort" placeholder="请选择">
                                ...
                                <el-option key="tags.count" label="SQL执行次数" value="tags.count"></el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
            ...
            <el-row>
                <el-col :span="24">
                    <el-table :data="tableData.rows" border stripe>
                        ...
                        <el-table-column prop="tags.count" label="sql执行" width="100">
                        </el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 柱状图数据
            queryChartData(){
                const url = "/api/tx/chart";
                let params = this.form;
                params.beginTime=this.getBeginTime();
                params.endTime=this.getEndTime();
                this.$axios.post(url, params).then((res) => {
                    console.log("==>queryTxChartData=%o",res);
            ...
            queryById(row,index,title) {
                if (index == "nvl" && row.type == "req") {
                    index = "lt-request-body";
                } else if (index == "nvl" && row.type == "proc") {
                    index = "lt-process-param";
