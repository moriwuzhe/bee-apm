...
async function openParamsByCallTreeNode(node: any) {
  ...
  let index = 'lt-request-body'
  if (type === 'proc') index = 'lt-process-param'
  if (type === 'sql') index = 'lt-sql-param'
  ...
  try {
    ...
    if (result && result.tags) {
      if (index === 'lt-process-param') content = result.tags.param
      else if (index === 'lt-sql-param') content = result.tags.args
...
<template>
  <PageShell title="请求查询" :subtitle="subtitle || undefined">
    ...
    <el-table-column label="操作" width="220" fixed="right" align="center">
      <template #default="{ row }">
        <el-button link type="primary" @click="openQueryById(row, 'lt-request-body', '入参')">入参</el-button>
        <el-button link type="primary" @click="openQueryById(row, 'lt-response-body', '回参')">回参</el-button>
